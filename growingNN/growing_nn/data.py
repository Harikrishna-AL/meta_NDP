...
def pad_grid(batch):
    ...
    padded_input_grids = [torch.nn.functional.pad(grid, (0, max_width - grid.shape[1], 0, max_height - grid.shape[0])) for grid in input_grids]
    padded_output_grids = [torch.nn.functional.pad(grid, (0, max_width - grid.shape[1], 0, max_height - grid.shape[0])) for grid in output_grids]

    return {'input_grids': torch.stack(padded_input_grids), 'output_grids': torch.stack(padded_output_grids)}

def get_data_loader(data_dir, batch_size, train=True):
    dataset = ARC(data_dir, train=train)
    data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, collate_fn=pad_grid)
    return data_loader
